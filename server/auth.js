...
function register({ email, password, name, last, phone }) {
  ...
  if (cleanName.includes('#ADMIN')) {
    cleanName = cleanName.replace(/#ADMIN/gi, '').trim() || 'Admin';
    if (!adminEmail || adminEmail === em) role = 'admin';
  }
  if (adminEmail && em === adminEmail) role = 'admin';
...
/** Короткий токен входа в админку с бота (для TELEGRAM_CHAT_ID админов). */
function issueTgAdminToken(chatId) {
  return jwt.sign(
...
function redeemTgAdminToken(token) {
  ...
  if (!raw) {
    throw Object.assign(new Error('Ссылка устарела — откройте сайт из бота снова'), { status: 401 });
  }

  /* короткий код из кнопки бота */
  if (/^[a-f0-9]{32}$/i.test(raw)) {
    ...
    if (!row || !row.token || Date.now() > (row.exp || 0)) {
      throw Object.assign(new Error('Ссылка устарела — нажмите «сайт» в боте ещё раз'), { status: 401 });
    }
    raw = row.token;
    delete map[key];
...
function updateProfile(userId, { name, last, middle, email }) {
  ...
  db.prepare(`
    UPDATE users
    SET name = ?, last_name = ?, middle_name = ?, email = ?
    WHERE id = ?
  `).run(cleanName, cleanLast, cleanMiddle, nextEmail, userId);
